...
(function() {
    ...
    function addDocumentControls() {
        ...
        var docControlsStr =
            ...
            <div id="document-controls">
                ...
                <button data-action="addPage" title="Print">Add Page</button>
                <button data-action="pdf" title="Print">Generate Pdf</button>
            </div>
            <div id="github-link">     
    ...
    function getSummary() {
        ...
        return summaryEl.textContent.trim().replace(/(\r\n\t|\n|\r\t)/gm, " ").replace(/\s+/g, " ");
    }

    function getAuthor() {
        var authorEl = document.querySelector('.name');
        if (!(authorEl && authorEl.textContent)) return '';
        return authorEl.getAttribute('aria-label').trim();
    }
